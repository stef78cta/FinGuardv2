...
**Versiune format:** v2.3 — **10 coloane obligatorii (A–J)**  
**Ultima actualizare:** Iunie 2026

> **v2.3 — schimbare validare per rând:** validarea per rând `Total sume = SI + Rulaj` a fost
> **eliminată** (presupunea rulaj LUNAR, dar balanțele reale pot avea Total Sume CUMULAT de la
> începutul anului). A fost înlocuită cu identitatea contabilă corectă, independentă de convenția
> rulajului: `(SF Debit − SF Credit) = (Total Sume Debitoare − Total Sume Creditoare)`.
> În plus, validările contabile per rând **nu mai exclud rândul din totaluri** — singurele rânduri
> excluse sunt cele structural invalide (cod lipsă/invalid, denumire prea lungă). Astfel se elimină
> dezechilibrele „fantomă” pe Sold inițial cauzate de rânduri respinse pe nedrept.
...
| G | `total_sume_debitoare` | **Total sume debitoare** (preluat ca atare; lunar sau cumulat) |
| H | `total_sume_creditoare` | **Total sume creditoare** (preluat ca atare; lunar sau cumulat) |
...
**Identitate contabilă blocking per rând (toleranță 0,01 RON):**
- `(SF Debit − SF Credit) = (Total Sume Debitoare − Total Sume Creditoare)`

> Notă: NU se mai verifică `Total Sume = SI + Rulaj curent`, pentru a accepta balanțele cu rulaj
> lunar și total sume cumulate de la începutul anului. Coloanele G/H sunt preluate ca atare din fișier.
...
| `BALANCE_ROW_CLOSING_MISMATCH` | (SF D − SF C) ≠ (Total Sume D − Total Sume C) | `Rândul {n}: soldul final net (SF Debit − SF Credit) nu corespunde cu Total Sume Debitoare − Total Sume Creditoare…` |
...
| Cod | Mesaj |
|---|---|
| `BALANCE_CLOSING_MISMATCH_DETECTED` | `{N} rând(uri) unde soldul final nu corespunde cu (Total Sume Debitoare − Total Sume Creditoare). Upload-ul a fost blocat.` |
| `BALANCE_INVALID_ROWS_DETECTED` | `{N} rând(uri) cu erori detectate: conturi lipsă sau invalide` |

Detalii per eroare identitate sold final (în `rowErrors[].details`):
...
```
❌ 3 rând(uri) unde soldul final nu corespunde cu (Total Sume Debitoare − Total Sume Creditoare). Upload-ul a fost blocat.
  • Total rânduri afectate: 3
  • Exemple:
    - Rândul 12, cont 401: closing_balance trebuie să fie total_sume_debitoare - total_sume_creditoare
...
| SF ≠ Total Sume D − Total Sume C | `BALANCE_CLOSING_MISMATCH_DETECTED` |
...
4. **(SF Debit − SF Credit) ≠ (Total Sume Debitoare − Total Sume Creditoare)** per rând (peste 0,01 RON)
5. Total SI / Rulaj / SF dezechilibrate global (peste 0,01 RON)
6. Conturi clasa 6/7 cu sold final nenul
7. Zero conturi valide
