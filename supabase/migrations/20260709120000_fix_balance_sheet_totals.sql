-- ============================================================================
-- Fix: total bs_1570, linii SLD lipsă, potrivire formule CALCULATED, functional_type
-- ============================================================================

-- 1) Corectează formula total pasive + capitaluri (potrivire exactă section_l1)
UPDATE public.statement_line_definitions
SET formula_or_rule = 'Datorii pe termen scurt + Datorii pe termen lung + Total capital propriu și quasi-capital'
WHERE statement_type = 'balance_sheet'
  AND line_key = 'bs_1570'
  AND company_id IS NULL;

-- 2) Linii leaf lipsă pentru conturi operaționale frecvente în balanțe reale
...
SELECT v.*
FROM (VALUES
(NULL::uuid, 'balance_sheet'::varchar, 'bs_215'::varchar, 215, 215, 'Avansuri acordate pentru imobilizări corporale',
 'Active', 'Active imobilizate', 'Imobilizări corporale (tangibile) nete', 'Imobilizări corporale (tangibile) brute', NULL::text,
 '4093', 'ACCOUNT_LEAF', 'account', 'Debit', '+', TRUE, NULL::text, 'Cont operațional 4093'),

(NULL::uuid, 'balance_sheet'::varchar, 'bs_296'::varchar, 296, 296, 'Împrumuturi nerambursabile cu caracter de subvenții',
 'Active', 'Active imobilizate', 'Creanțe pe termen lung / imobilizate', 'Creanțe imobilizate', NULL::text,
 '4452', 'ACCOUNT_LEAF', 'account', 'Debit', '+', TRUE, NULL::text, 'Sold debitor = activ; cont 4452'),

(NULL::uuid, 'balance_sheet'::varchar, 'bs_481'::varchar, 481, 481, 'Mărfuri aflate la terți',
 'Active', 'Active curente / circulante', 'Stocuri nete', 'Stocuri brute', NULL::text,
 '357', 'ACCOUNT_LEAF', 'account', 'Debit', '+', TRUE, NULL::text, NULL::text),

(NULL::uuid, 'balance_sheet'::varchar, 'bs_482'::varchar, 482, 482, 'Diferențe de preț la mărfuri',
 'Active', 'Active curente / circulante', 'Stocuri nete', 'Stocuri brute', NULL::text,
 '378', 'ACCOUNT_LEAF', 'account', 'Mixt', '+/-', TRUE, NULL::text, 'Bifuncțional; sold creditor reduce stocurile'),

(NULL::uuid, 'balance_sheet'::varchar, 'bs_483'::varchar, 483, 483, 'Ambalaje',
 'Active', 'Active curente / circulante', 'Stocuri nete', 'Stocuri brute', NULL::text,
 '381', 'ACCOUNT_LEAF', 'account', 'Debit', '+', TRUE, NULL::text, NULL::text),

(NULL::uuid, 'balance_sheet'::varchar, 'bs_606'::varchar, 606, 606, 'Decontări între entități afiliate (creanță)',
 'Active', 'Active curente / circulante', 'Creanțe nete', 'Creanțe brute', NULL::text,
 '4511', 'ACCOUNT_LEAF', 'account', 'Debit', '+', TRUE, NULL::text, 'Bifuncțional cu sold debitor'),

(NULL::uuid, 'balance_sheet'::varchar, 'bs_701'::varchar, 701, 701, 'Sume în curs de decontare',
 'Active', 'Active curente / circulante', 'Sold numerar', NULL::text, '512',
 '5125', 'ACCOUNT_LEAF', 'account', 'Debit', '+', TRUE, NULL::text, NULL::text),

(NULL::uuid, 'balance_sheet'::varchar, 'bs_1285'::varchar, 1285, 1285, 'Acționari/Asociați - conturi curente',
 'Pasive', 'Datorii pe termen scurt', 'Datorii comerciale curente către furnizori / clienți', NULL::text, NULL::text,
 '4551', 'ACCOUNT_LEAF', 'account', 'Credit', '+', TRUE, NULL::text, 'Datorie față de asociați/acționari'),

(NULL::uuid, 'balance_sheet'::varchar, 'bs_1295'::varchar, 1295, 1295, 'Decontări între entități afiliate (datorie)',
 'Pasive', 'Datorii pe termen scurt', 'Datorii comerciale curente către furnizori / clienți', NULL::text, NULL::text,
 '4511', 'ACCOUNT_LEAF', 'account', 'Credit', '+', TRUE, NULL::text, 'Bifuncțional cu sold creditor'),

(NULL::uuid, 'balance_sheet'::varchar, 'bs_1435'::varchar, 1435, 1435, 'Împrumuturi nerambursabile cu caracter de subvenții pentru investiții',
 'Pasive', 'Total capital propriu și quasi-capital', 'Quasi-capitaluri', 'Subvenții', '475',
 '4752', 'ACCOUNT_LEAF', 'account', 'Credit', '+', TRUE, NULL::text, 'Subvenții pentru investiții — cont 4752')
) AS v(
    company_id, statement_type, line_key, sort_order, display_order, display_name,
    report_area, section_l1, section_l2, section_l3, section_l4,
    account_code, row_type, line_type, normal_balance, report_sign,
...
-- 3) Sold de raportare: prioritate functional_type față de account_type
...
AS $$
    SELECT CASE
        ...
        WHEN COALESCE(_functional_type, '') = 'bifunctional' THEN
            CASE
                WHEN _debit > _credit THEN (_debit - _credit)
                WHEN _credit > _debit THEN (_credit - _debit)
                ELSE 0
            END
...
-- 4) Regenerare situații financiare — potrivire formule + conturi bifuncționale + functional_type
        ...
        JOIN LATERAL (
            ...
            WHERE l2.statement_type = 'balance_sheet'
              ...
              AND (
                  ...
                  OR (
                      m.closing_credit > m.closing_debit
            ...
            ORDER BY
                ...
                CASE
                    WHEN COALESCE(m.functional_type, '') = 'bifunctional'
                         AND m.closing_debit >= m.closing_credit
                         AND l2.report_area = 'Active' THEN 0
                    WHEN COALESCE(m.functional_type, '') = 'bifunctional'
                         AND m.closing_credit > m.closing_debit
